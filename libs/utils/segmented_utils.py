...
def train_one_epoch(
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        for i in range(len(video_list)):
          ...
          for j in range(0,v2.shape[0],10):
            print('j', j)
            #print(type(v))
            v = v2[j:j+10, :, :, :, :]
            ...
            feats[ :1024 , frames[j:j+10] ] = newfeats.squeeze().t().cpu() # does this write straight to the dict?
            savefeats[video_list[i]['fileframes'][j:j+10], :1024] = newfeats.squeeze().detach().cpu().numpy()
