    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            for i in range(len(video_list)):
              ...
              feats = video_list[i]['feats']
              print(v.shape)
              for j in range(0, v.shape[0], 250):
                  newfeats = extractor(v[j:min(j+250, feats.shape[1]), :])
                  newfeats = newfeats.squeeze().t()
                  print(newfeats.shape)
                  feats[ :1024, j:min(j+250, feats.shape[1]) ] =newfeats[:, :]
              video_list[i]['feats'] = feats
            # update video list only, not underlying file
            print("creating model output")
