...
def make_optimizer(model, extractor, optimizer_config):
    """create optimizer
    ...
    optim_groups = [
        ...
        {"params": extractor.parameters(), "weight_decay": 0.0, "lr":optimizer_config["learning_rate"]/10.0},
    ]
...
def train_one_epoch(
    ...
    print('Device at runtime', device)
    ...
    for iter_idx, video_list in enumerate(train_loader, 0):
        ...
        for i in range(len(video_list)):
            v = video_list[i]['raw']
            codex.append(codex[-1] + v.shape[0])
        clipstack = torch.empty(codex[-1], 3,16,224,224)
        #print(codex, clipstack.shape)


        for i in range(len(video_list)):
            v = video_list[i]['raw']
            #print(v.shape)
            #print(codex[i], codex[i+1])
            clipstack[codex[i]:codex[i+1], :] = v
        #print(clipstack.shape)
        clipstack = clipstack.to(torch.device('cuda:1'))
        #print(clipstack.device)
        #print(extractor._model.Conv3d_2b_1x1.conv3d.weight.device)
        #print(clipstack.shape)
        extractedfeats = extractor(clipstack)
        for i in range(len(video_list)):
            newfeats = extractedfeats[codex[i]:codex[i+1], :]
...
def valid_one_epoch(
    ...
    for iter_idx, video_list in enumerate(val_loader, 0):
        ...
        with torch.no_grad():
            # TODO extract new features from video list using second model
            #v = video_list['raw']
            #newfeats = extractor(raw)
            #video_list['feats'] = newfeats
            # update video list only, not underlying file
